...
class User_profile(db.Model, SerializerMixin):
    ...
    id=db.Column(db.String,unique=True,nullable=False,primary_key=True, default=lambda:str(uuid.uuid4()))
...
class Driver(db.Model,SerializerMixin):
    ...
    id=db.Column(db.String,default=lambda:str(uuid.uuid4()),unique=True,nullable=False, primary_key=True)
    phone_number=db.Column(db.Integer,unique=True,nullable=False)
    password=db.Column(db.String, nullable=False)
    mark_deleted=db.Column(db.Boolean, nullable=False)
...
class Models(db.Model,SerializerMixin):
    ...
    id=db.Column(db.String, unique=True, primary_key=True, default=lambda:str(uuid.uuid4()))
...
class Make(db.Model,SerializerMixin):
    ...
    model_id=db.Column(db.String,db.ForeignKey("models.name"))
...
class Vehicle(db.Model,SerializerMixin):
    ...
    make_name=db.Column(db.String,db.ForeignKey("make.id"),nullable=False)
...
class Order(db.Model,SerializerMixin):
    ...
    id=db.Column(db.String, primary_key=True, nullable=False, unique=True, default=lambda:str(uuid.uuid4()))
    ...
    #serialise rules
    serialize_rules=("-merchant.orders","-driver.deliveries","-recipient.deliveries")
